...
bound to the watcher lifecycle runs. When a watch is triggered, watcher ensures that each
watch is only triggered on one of those shards, with all primaries and replicas
taken into account.  The more replica shards you
...
Because watches are executed on the node where the watch shards are, you can create
